...
function _FillDataDays()
{
	...
	//	console.log(".fc-title div."+ _ADJUNTO + "[id=" + ID_INICIAL +"]");
			...
			/* FIN ESTO ES PARA EVITAR QUE NO ME CONTABILICE LOS DIAS DE OTROS MESES */
			 ...
			 var bEsPresencia=false;	
		      ...
		      if ($(this).hasClass(_PRESENCIA))
		      {
		    	  ...
		    	  bEsPresencia=true; // nos vvale para no computar refuerzos ni localizadas en los simulados
		      }
		      // simulado es si contiene el class simulado en alguno de los residentes asociados
		      // class="orden3 adjunto presencia nosimulado 
		...
		if (ID_INICIAL!=-1)
		{				
			$("#doctor_list table tr[id=" +ID_INICIAL +"] td.mespresencia").html("<span class=\"badge\">" + Presencia + "</span>");			
			$("#doctor_list table tr[id=" +ID_INICIAL +"] td.meslocalizada").html("<span class=\"badge\">" + Localizada + "</span>");
			$("#doctor_list table tr[id=" +ID_INICIAL +"] td.mesrefuerzo").html("<span class=\"badge\">" + Refuerzo + "</span>");
			$("#doctor_list table tr[id=" +ID_INICIAL +"] td.mespresenciaf").html("<span class=\"badge\">" + PresenciaF + "</span>");			
			$("#doctor_list table tr[id=" +ID_INICIAL +"] td.meslocalizadaf").html("<span class=\"badge\">" + LocalizadaF + "</span>");
			$("#doctor_list table tr[id=" +ID_INICIAL +"] td.mesrefuerzof").html("<span class=\"badge\">" + RefuerzoF + "</span>");
			$("#doctor_list table tr[id=" +ID_INICIAL +"] td.mestotaladjunto").html("<span class=\"badge\">" + (Presencia+Localizada+Refuerzo+PresenciaF+LocalizadaF+RefuerzoF) + "</span>");
			$("#doctor_list table tr[id=" +ID_INICIAL +"] td.messimulados_adjunto").html("<span class=\"badge\">" + Simulados + "</span>");
		...
		$("#doctor_list table tr[id=" +ID_INICIAL +"] td.mestotalresidente").html("<span class=\"badge\">" + Guardias + "</span>");
		$("#doctor_list table tr[id=" +ID_INICIAL +"] td.mesfestivos").html("<span class=\"badge\">" + FestivosR + "</span>");
		$("#doctor_list table tr[id=" +ID_INICIAL +"] td.mesdiario").html("<span class=\"badge\">" + _totalDiario  + "</span>");
...
function fSaveDatabaseOrExcel(Operation){
  			  	...
  			  	if (Operation!='excelmail')
  			  	{
  			  	   ...
  			  	   _fn_OrdenarSecuenciaAdjuntos(LastIDADJUNTOPRESENCIA);
  			  		...
  			  		$('#loaded').modal('show');
  						...
  						window.location.href=_REQUEST_CONTEXT + 'login.jsp';
	...
	/* 
	<li><a onclick =_state('<%=Util.eEstadoCambiosGuardias.APROBADA%>',<%=oCambio.getIdCambio()%>) href="#">SI,CESION</a></li>
    <li><a onclick =_state('<%=Util.eEstadoCambiosGuardias.APROBADA%>',<%=oCambio.getIdCambio()%>) href="#">SI,INTERCAMBIO</a></li>
	...
	*/
	...
	function _state (newstate, idchange, type,destinatario)
	{
		...
		$( "#confirmar_cambio" ).load( "medico/_list_medicos_cambio_guardia.jsp?cambioid="  + idchange  + '&newstate=' + newstate  + '&type=' + type);
	...
	function _changeState (newstate, idchange,MedicoDestination, Type)
	{
		  ...
		  $.ajax({
	          ...
	          complete: function(data) {	        	  	        	  
	        	  ...
	        	  $('#' + idchange + '_aprobacion').html(moment().format("YYYY-MM-DD"));

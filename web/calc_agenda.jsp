	...
	for (int x=0;x<_lAdjuntos.size();x++)
	{
				...
				&& (lGenerada.size()==0 || !lGenerada.contains(oM.getID())))
		{
			...
			{			
				...
				int TOTAL_PRESENCIA = GuardiasDBImpl.getTotalGuardiasPorMedicoTipo(oM.getID(), Util.eTipoGuardia.PRESENCIA.toString().toLowerCase(),new Long(0));	
				int TOTAL_LOCALIZADA = GuardiasDBImpl.getTotalGuardiasPorMedicoTipo(oM.getID(), Util.eTipoGuardia.LOCALIZADA.toString().toLowerCase(),new Long(0));
				int TOTAL_REFUERZO = GuardiasDBImpl.getTotalGuardiasPorMedicoTipo(oM.getID(), Util.eTipoGuardia.REFUERZO.toString().toLowerCase(),new Long(0));						
...
if (ProcesarMedicos.ExisteMedicoConGuardiaSolo_EnElMes(lMedicosGuardias, _lAdjuntos, new Long(-1)))
{
	for (int j=1;j<=_daysOfMonth;j++)	  
	{
...
for (int j=1;j<=NUM_SEMANAS_MES;j++)
{
	...
	//System.out.println("_mapDiasSemana:" + _mapDiasSemana.entrySet().size());
    ...
    for (Map.Entry<Long,Util.eTipoDia> DiaSemana : _mapDiasSemana.entrySet())
	{
		...
		if (!_lPoolDAYS.contains(DIASEMANAMES) && !bExisteMedicoConGuardiaSolo)
		{
		    ...
		    */
				...
				if (oM.isResidenteSimulado())
				 {
					...
					/*	 
						if  (bAdjuntoConMenosSimulados &&  !_EsFestivo &&  !ExcedeLimiteGuardiasMes && !ExcedeHorasSeguidas) // no cumpla las condiciones, evitamos festivos 
						{						
								bEncontrado =true;
						}					
			...
			//System.out.println("Dia:" + DIASEMANAMES + "TotalGuardias:" + _lDatosGuardiasMedico.get("_NUMERO_GUARDIAS_" + Util.eTipo.RESIDENTE + "_MES") + ",Festivos:" + _lDatosGuardiasMedico.get("_TOTAL_GUARDIAS_RESIDENTE_FESTIVO"));
...
for (int j=1;j<=_daysOfMonth;j++)	
{
		...
		Long IDResidenteGuardiaDia = new Long(-1);
		Long IDAdjuntoGuardiaDia = new Long(-1);
		
		
		
		IDResidenteGuardiaDia = ProcesarMedicos.getMedicoGuardiaDia(j, lMedicosGuardias, Util.eTipo.RESIDENTE, _lResidentes);    
		//PRESENCIA 
		IDAdjuntoGuardiaDia = ProcesarMedicos.getMedicoGuardiaDia(j, lMedicosGuardias, Util.eTipo.ADJUNTO, _lAdjuntos);
		
		if (IDResidenteGuardiaDia.intValue()==-1)
		{
		...
		Util.eTipoGuardia _TipoGuardiaAdjunto = Util.eTipoGuardia.REFUERZO;
		/* R4, R5 */
		...
		while (!bEncontrado) 
		{
			...
			*/
			...
			EstaPresenciaEseDia  = IDAdjuntoGuardiaDia.equals(oM.getID());
			...
			if (!EstaPresenciaEseDia && lMedicosGuardias.containsKey(oM.getID())) // existe el medico con guardias
			{
				...
				_lDatosTemp = (Hashtable) lMedicosGuardias.get(oM.getID());
				 ...
				 AdjuntoConMenosGuardias = ProcesarMedicos.AdjuntoMenosGuardiasYHorasSeguidas(false, _TipoGuardiaAdjunto, _Key, lMedicosGuardias, _lAdjuntos, IDAdjuntoGuardiaDia,j, _daysOfMonth);
		...
		System.out.println("Agregado al dia : " + j + ", médico adjunto:" + oM.getID() + "," + _TipoGuardiaAdjunto + " " + _Key + ",Adjunto de Guardia:" + IDAdjuntoGuardiaDia);
	 ...
	 while (entries.hasNext()) 
		...
		// CAMBIO 20161226, INCLUIMOS EN LOS RESULTADOS A LOS RESIDENTES
		  ...
		  if (lDatosTemp.containsKey(new Long(j)))   // encuentra el dia 
		  {
			  ...
			  _lDatos.put(_Temp2, _oM.getID()  + "|" + _oM.getApellidos() + "|" + _oM.getNombre());
	 ...
	 while (entries.hasNext()) 
		{
		 ...
		 String _DATE = _format.format(cGuardiaDia.getTime());
		  ...
		  while (entriesGuardias.hasNext())
		  {
			  ...
			  _EventsJSON += "<div class='orden" + classContador + " " + _classTipo +  " " + _sGuardia.toString().toLowerCase() +" " + sFestivo + "' id=" + _GuardiaMedicoID + ">"   + _GuardiaApellidos + "," + _GuardiaNombre + "</div>";
			  classContador+=1;
		  }
